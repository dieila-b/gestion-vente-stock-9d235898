...
import { Package, Wallet, Receipt, TrendingUp, CreditCard, ShoppingBag, LineChart, Boxes, PiggyBank, BaggageClaim } from "lucide-react";
    ...
    <>
      <div className="grid gap-4 md:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 animate-fade-in delay-200">
        ...
        <StatsCard
          ...
          value={`${formatGNF(unpaidAmount)} FG`}
          icon={Receipt}
      ...
      <div className="grid gap-3 md:gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-6 animate-fade-in delay-300">
        <StatsCard
          ...
          value={catalogLength.toString()}
          icon={Package}
          onClick={() => navigate('/catalog')}
          className="bg-gradient-to-br from-green-500/10 to-green-600/10 py-1 h-[80px]"
        />
        <StatsCard
          title="Stock Global"
          ...
          className="bg-gradient-to-br from-indigo-500/10 to-indigo-600/10 py-1 h-[80px]"
        />
        <StatsCard
          title="Valeur Stock (Achat)"
          value={`${formatGNF(totalStockPurchaseValue)} FG`}
          icon={ShoppingBag}
        ...
        <StatsCard
          ...
          value={`${formatGNF(totalStockSaleValue)} FG`}
          icon={BaggageClaim}
        ...
        <StatsCard
          ...
          value={`${formatGNF(globalStockMargin)} FG`}
          icon={PiggyBank}
